...
#     folium.Marker(location=[row['위도'], row['경도']], popup='광진구 가로등', icon=folium.Icon(color='red')).add_to(m)
for idx, row in smart_street_lamp_data_ll.iterrows():
    folium.Marker(location=[row['위도'], row['경도']], popup='광진구 스마트 가로등', icon=folium.Icon(color='orange')).add_to(m)

#m.show_in_browser()
m.save("map_with_markers_smart_street_lamp.html")